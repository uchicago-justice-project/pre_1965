...
gap_outliers <- read_csv("data/raw/waves/gap_outliers.csv", show_col_types = FALSE, progress  = FALSE) %>% 
  dplyr::select(GRID_ID, year, last_wave_end, longest_gap) %>% 
  filter(year > last_wave_end & year <= last_wave_end + longest_gap)

gap_outliers %>% 
  filter(year >= 1940 & year <= 1955) 

outliers_geom <- df %>% 
  dplyr::select(GRID_ID, year) %>% 
  inner_join(gap_outliers, by = c("GRID_ID", "year")) 

#increases_count_rate <- read_csv("data/raw/waves/increases_count_rate.csv", show_col_types = FALSE, progress  = FALSE)
...
for (year_select in years) {
  ...
  g <- ggplot() +
    ...
    geom_sf(data = streets_select, aes(color = paste0(DEDICATED_, ", Construction Estimated ", begin, 
                                                      "\nOpening Date ", open))) + 
    geom_sf(data = outliers_plot, alpha = 0.4, fill = "red") +
    theme_minimal() +
