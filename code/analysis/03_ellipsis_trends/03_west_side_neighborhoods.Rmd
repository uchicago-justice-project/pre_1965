...
reg <- glm(homicides ~ total_pop+ SHAPE_AREA, family = poisson(), data = census_west_homs_geom)
summary(reg)

census_west_homs_geom$pred_homicides_pop_size <- predict(reg, type = "response")
census_west_homs_geom$res_homicides_pop_size <- residuals(reg, type = "response")

reg <- glm(homicides ~ SHAPE_AREA, family = poisson(), data = census_west_homs_geom)
summary(reg)

census_west_homs_geom$pred_homicides_size <- predict(reg, type = "response")
census_west_homs_geom$res_homicides_size <- residuals(reg, type = "response")


reg <- feglm(homicides ~ total_pop + SHAPE_AREA + share_under_25 + share_black*share_over_1_ppr*share_under_25 
            + share_foreign*share_over_1_ppr*share_under_25 
            + vacant_homes + unemployment_rate + college_grads + median_value   |  year
            ,family = poisson(), data = census_west_homs_geom)
census_west_homs_geom$res_homicides_all <- residuals(reg, type = "response")
...
saveRDS(census_west_homs_geom, "output/westside_interactive_maps/census_west_homs_geom.rds")
saveRDS(homs_west, "output/westside_interactive_maps/homs_west.rds")
saveRDS(projects_west, "output/westside_interactive_maps/projects_west.rds")
saveRDS(westside, "output/westside_interactive_maps/westside.rds")

save.image("output/westside_interactive_maps/my_workspace.RData")
```
