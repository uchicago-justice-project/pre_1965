...
p <- ggplot(data = all_ellipses, aes(x = year, y = -current_x)) +
  ...
  scale_y_continuous(labels = function(x) paste0(format(x, nsmall  = 2), "°W"), limits = c(87.665, 87.7), expand = c(0, 0)) +
  theme_classic()
...
for (i in 1:3) {
 ...
 year <- years[i]
  ...
  p1 <- ggplot() + 
    geom_sf(data = census_west_homs_geom %>% filter(year == !!year), aes(fill = homicide_per_area)) + 
    theme_void() + 
    scale_fill_gradient( low = "black", high = "white", guide = guide_colorbar(barheight = 1)
                         , limits = c(0, 0.00015),  breaks = c(0, 7.5e-05, 0.00015)) + 
  ...
  p2 <- ggplot() + 
    ...
    theme_void() + 
    scale_fill_gradient( low = "black", high = "white", guide = guide_colorbar(barheight = 1) , limits = c(0, 0.7)) + 
  ...
  p3 <- ggplot() + 
    geom_sf(data = census_west_homs_geom %>% filter(year == !!year), aes(fill = male_15_24_per_area)) + 
    theme_void() + 
    scale_fill_gradient( low = "black", high = "white", guide = guide_colorbar(barheight = 1), limits = c(0, 0.005 )) + 
