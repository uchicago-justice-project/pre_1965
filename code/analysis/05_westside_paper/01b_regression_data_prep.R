...
tracts_tenthmile_highway <- census_walk %>% 
  filter(year == 1950) %>% 
...
tracts_quartmile_highway <- census_walk %>% 
  filter(year == 1950) %>% 
  dplyr::select(GISJOIN_1970) %>% 
...
tracts_halfmile_highway <- census_walk %>% 
  filter(year == 1950) %>% 
  dplyr::select(GISJOIN_1970) %>% 
...
tracts_1mile_highway <- census_walk %>% 
  filter(year == 1950) %>% 
  dplyr::select(GISJOIN_1970) %>% 
...
st_write(reg_data, "data/mst/for_paper/reg_data.geojson", append = FALSE )
